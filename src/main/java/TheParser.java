...
public class TheParser {
	...
	public void RULE_SWITCH() {
		...
		while (tokens.get(currentToken).getValue().equals("case")) {
			...
			//  Parse the case body (using RULE_BODY to consume a sequence of statements until a break, next case/default, or closing '}' is reached)
			RULE_BODY();
	...
	// RULE_WHILE, RULE_DO_WHILE, RULE_SWITCH remain unchanged.
	...
	public void RULE_IF() {
		...
		if (tokens.get(currentToken).getValue().equals("{")) {
			...
			System.out.println("-- Found 'else' for one line IF body");
			currentToken++;
			RULE_BODY();
			return;
		}
		...
		if (tokens.get(currentToken).getValue().equals("else")) {
			...
			RULE_BODY();
			if (tokens.get(currentToken).getValue().equals("}")) {
				System.out.println("-- Found '}' after ELSE body");
				currentToken++;
			} else {
				error(60);
			}
	...
	// RULE_DO_WHILE and RULE_SWITCH methods (omitted for brevity) remain similar.
